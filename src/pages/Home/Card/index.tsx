...
const Card = ({ title, type, time, image, color }: CardProps): JSX.Element => (
  <div className={`flex flex-col gap-4 px-4 py-2 rounded-xl border-2 ${color}`}>
    {image && <img src={image} alt="card" className="rounded-xl" />}
    ...
    {time && <p className="text-2xl font-bold">{time}</p>}
  </div>
);
